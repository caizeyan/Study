			...
			fixed4 frag(v2f i) : SV_Target {
				...
				fixed3 specular = _LightColor0*_Specular*pow(saturate(dot(worldNormal,halfView)),_Gloss);
					...
					fixed atten = 1;
						...
						//tex2D(_LightTexture0,lightCoord.xy/lightCoord.w + 0.5).w  与聚光灯角度相关
						//tex2D(_LightTextureB0,dot(lightCoord,lightCoord).rr).UNITY_ATTEN_CHANNEL  与聚光灯的距离相关
